...
class Material:
    def __init__(self, v_or: float, og: float, ob: float, kd: float, ks: float, ka: float, kr: float, kt: float, p: float, refracao: float):
        ...
        with open(filename, "r") as file:
            for line in file.readlines():
                ...
                if content[0] == 'c':
                    ...
                    self.camera = [camera.ponto_observador, camera.ponto_alvo, camera.vetor_up, -1, camera.distancia, camera.largura, camera.altura]
                if content[0] == 's':
                    sphere = Sphere(
                        np.array([int(content[1]), int(content[2]), int(content[3])]), 
                        int(content[4]),
                        Material(float(content[5]), float(content[6]),float(content[7]), float(content[8]), float(content[9]), float(content[10]), float(content[11]), float(content[12]), float(content[13]), float(content[14])),
                    )
                ...
                if content[0] == 'p':
                    plane = Plane(
                        np.array([int(content[4]), int(content[5]), int(content[6])]),
                        np.array([int(content[1]), int(content[2]), int(content[3])]),
                        Material(float(content[7]), float(content[8]),float(content[9]), float(content[10]), float(content[11]), float(content[12]), float(content[13]), float(content[14]), float(content[15]), float(content[16])),
                    )
                ...
                if n_triangles == 0 and read_mesh_material:
                    ...
                    for triangle in triangles:
                        self.objetos[self.objetos_index] = [
                            'triangulo', triangle.ponto1, triangle.ponto2, triangle.ponto3, triangle.normal, triangle.indexes, normal_vertices,
                            [float(content[0]), float(content[1]), float(content[2]), float(content[3]), float(content[4]), float(content[5]), float(content[6]), float(content[7]), float(content[8]), float(content[9])],
                        ]
                        self.objetos_index += 1
